...
#create blockmodel with 5 blocks and enforcing connectivity if necessary
...
def adjacency(clustersizes, wiring_probs, connected=True):
    ...
    connected: boolean
        if True, a connected network is enforced
    ...
    for i in range(N):
        for j in np.arange(i+1,N): #no self-loops
            ...
            #rewire with the corresponding probability
