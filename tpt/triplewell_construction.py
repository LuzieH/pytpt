...
ind_C = np.argwhere(np.array([set_C_triplewell(grid[:,i],A_center, \
        B_center,radius_setAB) for i in \ 
        np.arange(np.shape(xn)[0])])==1).squeeze()

# indices of B
ind_B = np.argwhere(np.array([set_B_triplewell(grid[:,i],B_center, \ 
        radius_setAB) for i in np.arange(np.shape(xn)[0])])==1).flatten()

# indices of B
ind_A = np.argwhere(np.array([set_A_triplewell(grid[:,i],A_center, \
        radius_setAB) for i in np.arange(np.shape(xn)[0])])==1).flatten()
