...
if create_new_example == True:
    ...
    while x[0] != 39 or x[1] != 39:
        ...
        moves = np.array([np.array([-1, 0]), np.array([1, 0]),
    ...
    T = np.load(os.path.join(my_path, 'data/P_maze.npy'))
...
# all possible states -> gives the state space of size st
xy_pot = np.zeros((st, 2))
count = 0
for i in range(S):
    for j in range(S):
        if pot[i, j] == 0:
