...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  6 13:35:46 2019
...
class transitions_periodic:
    ...
    def stationary_density(self):
        ...
        # normalize
        stat_dens[0,:] = (np.real(eig[:,index])/np.sum(np.real(eig[:,index]))).flatten()
        ...
        for m in range(self._M):    
            ...
            for i in np.arange(self._S):
                for j in np.arange(self._S):
                     ...
                     self._stat_dens[np.mod(m-1,self._M),i]/self._stat_dens[np.mod(m,self._M),j]
    ...
    def committor(self):
        ...
        D = np.diag(np.ones(dim_C)) # multiplied transition matrix over period with only transitions in C
        b = np.zeros(dim_C) # remaining part of the equation    
              
        # filling D and b
        for m in np.arange(self._M):

            b = b + (D.dot(self._P(m)[np.ix_(self._ind_C, self._ind_B)])).dot(np.ones(dim_B))
        ...
        for m in np.arange(1,self._M)[::-1]:
            q_f[m,self._ind_C] = self._P(m)[self._ind_C,:].dot(q_f[np.mod(m+1,self._M),:]) 
        ...
        for m in times:    
            ...
            a = a + (D_back.dot(self._P_back(m)[np.ix_(self._ind_C, self._ind_A)])).dot(np.ones(dim_A))
            D_back = D_back.dot(self._P_back(m)[np.ix_(self._ind_C, self._ind_C)])
        ...
        for m in np.arange(1,self._M):
            q_b[m,self._ind_C] = self._P_back(m)[self._ind_C,:].dot(q_b[m-1,:])             
    ...
    def reac_density(self):
        ...
        for m in range(self._M):
            ...
            reac_dens[m,:] = np.multiply(self._q_b[m,:],np.multiply(self._stat_dens[m,:],self._q_f[m,:]))
            reac_dens[m,:] = reac_dens[m,:]/np.sum(reac_dens[m,:]) #normalization
    ...
    def reac_current(self):
        ...
        for m in range(self._M):
            ...
            for i in np.arange(self._S):
                for j in np.arange(self._S):
                    current[m,i,j] = self._stat_dens[m,i]*self._q_b[m,i]*self._P(m)[i,j]*self._q_f[m,j]
                    
                    if i+1>j:
                        eff_current[m,i,j] = np.max([0,current[m,i,j]-current[m,j,i]])
                        eff_current[m,j,i] = np.max([0,current[m,j,i]-current[m,i,j]])
