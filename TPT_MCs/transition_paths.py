...
class transitions_mcs:
    ...
    def __init__(self, P,  ind_A, ind_B,  ind_C, stat_dens=None):
        ...
        self._ind_B=ind_B
        self._ind_C=ind_C
        self._S=np.shape(self._P)[0] #size of the state space
        
        self._P_back=None #transition matrix of the time-reversed process
    ...
    def stationary_density(self):
        ...
        index=np.where(np.abs(eigv-1)<10e-4)[0]
        #normalize
        stat_dens=(np.real(eig[:,index])/np.sum(np.real(eig[:,index]))).flatten()
    ...
    def committor(self):
        ...
        for i in np.arange(self._S):
            for j in np.arange(self._S):
                 P_back[j,i]=self._P[i,j]*self._stat_dens[i]/self._stat_dens[j]
        self._P_back=P_back
        
        #transition matrices from states in C to C
        P_C=self._P[np.ix_(self._ind_C,self._ind_C)] #forward
        P_back_C=self._P_back[np.ix_(self._ind_C,self._ind_C)] #backward in time
        ...
        q_f_C=np.zeros(int(np.size(self._ind_C)))
        ...
        q_f[np.ix_(self._ind_C)] =q_f_C
        
        #compute backward committor on C  
        q_b_C=np.zeros(int(np.size(self._ind_C)))
    ...
    def reac_density(self):
        ...
        reac_dens=np.multiply(self._q_b,np.multiply(self._stat_dens,self._q_f))
    ...
    def reac_current(self):
        ...
        for i in np.arange(self._S ):
            for j in np.arange(self._S ):
                current[i,j]=self._stat_dens[i]*self._q_b[i]*self._P[i,j]*self._q_f[j]
                if i+1>j:
