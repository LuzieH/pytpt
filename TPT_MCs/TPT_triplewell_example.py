...
fig2 = plt.figure()
plt.imshow(np.reshape(q_f,(S_shape)), extent=[-2,2,-3,3])
plt.title('Forward committor')
plt.colorbar()

fig3 = plt.figure()
plt.imshow(np.reshape(q_b,(S_shape)), extent=[-2,2,-3,3])
...
fig4 = plt.figure()
plt.imshow(np.reshape(reac_dens,(S_shape)), extent=[-2,2,-3,3])
plt.title('Reactive density')
plt.colorbar()

fig5  = plt.figure()
plt.title('Reactive channels')
plt.imshow(np.reshape(current_dens,(S_shape)), extent=[-2,2,-3,3])
plt.colorbar()
